...
class HeartopiaGuide extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: ThemeData(colorSchemeSeed: Colors.orange, useMaterialDesign: true),
      home: const HomePage(),
    );
  }
}
...
class _HomePageState extends State<HomePage> {
  ...
  Widget build(BuildContext context) {
    ...
    double progress = entries.isNotEmpty ? collected / entries.length : 0;
          ...
          bottom: PreferredSize(
            ...
            child: Column(
              children: [
                ...
                const TabBar(tabs: [Tab(text: '魚類'), Tab(text: '蟲類'), Tab(text: '鳥類')]),
              ],
            ),
          ),
        ),
        body: TabBarView(
          children: [_buildList('魚類'), _buildList('蟲類'), _buildList('鳥類')],
        ),
      ),
    );
  }
      ...
      itemBuilder: (context, index) {
        ...
        return Card(
          ...
          child: CheckboxListTile(
            ...
            onChanged: (bool? value) => setState(() => item.isCollected = value ?? false),
          ),
        );
      },
    );
